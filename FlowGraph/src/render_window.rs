...
impl RenderWindow {
    ...
    pub fn screen_to_world_bounding_box(&self, bounding_box: ScreenBoundingBox) -> WorldBoundingBox {
        let x = self.screen_to_world_x(bounding_box.0);
        let y = self.screen_to_world_y(bounding_box.1);

        let w = self.screen_to_world_distance_x(&bounding_box.2);
        let h = self.screen_to_world_distance_y(&bounding_box.3);

        WorldBoundingBox(x,y,w,h)
    }
    ...
    pub fn screen_to_world_coords(&self, screen_coords: &ScreenCoords) -> WorldCoords {
    ...
    pub fn world_to_screen_coords(&self, world_coords: &WorldCoords) -> ScreenCoords {
    ...
    pub fn world_to_screen_bounding_box(&self, world_bounding_box: &WorldBoundingBox) -> ScreenBoundingBox  {
        let ScreenCoords(x, y) = self.world_to_screen_coords(&WorldCoords(world_bounding_box.0, world_bounding_box.1));
        ...
        ScreenBoundingBox(x,y, width, height)
    }
    ...
    pub fn world_to_render(&self, world_coords: &WorldCoords) -> RenderCoords {
        self.screen_to_render(&self.world_to_screen_coords(world_coords))
    }
    ...
    pub fn zoom_window(&mut self, center: &ScreenCoords, direction: ScrollDirection, mut delta: f64) {
        ...
        let point = self.screen_to_world_coords(center);
